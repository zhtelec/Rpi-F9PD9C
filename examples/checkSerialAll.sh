...
setBaud() {
    log=`ubxtool -w 0.1 -f $1 -z "CFG-$2-BAUDRATE,$baud"`
...
echo \# set baud
echo -n " 1. set $baud to CFG-UART1-BAUDRATE (${strSerial[0]}): "
setBaud /dev/ttyACM0 UART1
echo -n " 2. set $baud to CFG-UART2-BAUDRATE (${strSerial[4]}): "
setBaud /dev/ttyACM0 UART2
echo -n " 3. set $baud to CFG-UART1-BAUDRATE (${strSerial[1]}): "
setBaud /dev/ttyACM1 UART1
echo -n " 4. set $baud to CFG-UART2-BAUDRATE (${strSerial[3]}): "
...
do
    ...
    log=`ubxtool  -w 0.2  -f /dev/ttyAMA$i -s $baud -p MON-VER | grep VER`
    echo $log
done
